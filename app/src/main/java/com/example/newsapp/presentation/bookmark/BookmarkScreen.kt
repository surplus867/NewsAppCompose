package com.example.newsapp.presentation.bookmark

import androidx.compose.foundation.layout.Column
import androidx.compose.foundation.layout.Spacer
import androidx.compose.foundation.layout.fillMaxSize
import androidx.compose.foundation.layout.height
import androidx.compose.foundation.layout.padding
import androidx.compose.foundation.layout.statusBarsPadding
import androidx.compose.material3.MaterialTheme
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.res.colorResource
import androidx.compose.ui.text.font.FontWeight
import com.example.newsapp.R
import com.example.newsapp.domain.model.Article
import com.example.newsapp.presentation.BookmarkState
import com.example.newsapp.presentation.onboarding.Dimens
import com.example.newsapp.presentation.onboarding.common.ArticlesList

// Composable function for the BookmarkScreen
@Composable
fun BookmarkScreen(
    state: BookmarkState,
    navigateToDetails: (Article) -> Unit
) {
    // Column layout to organize UI elements vertically
    Column(
        modifier = Modifier
            .fillMaxSize() // Occupy the entire available height and width
            .statusBarsPadding() // Add padding to accommodate the system status bar
            .padding(top = Dimens.MediumPadding1, start = Dimens.MediumPadding1, end = Dimens.MediumPadding1)
        // Add padding to the top, start, and end sides
    ) {
        // Text element displaying "Bookmark" as the screen title
        Text(
            text = "Bookmark",
            style = MaterialTheme.typography.displayMedium.copy(fontWeight = FontWeight.Bold),
            color = colorResource(id = R.color.text_title)
        )

        // Spacer element to add vertical space
        Spacer(modifier = Modifier.height(Dimens.MediumPadding1))

        // ArticlesList composable to display a list of bookmarked articles
        // onClick parameter is a lambda function to handle item click events
        ArticlesList(articles = state.articles, onClick = {navigateToDetails(it)})
    }
}